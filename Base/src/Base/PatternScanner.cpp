...
void PatternScanner::ParseSignatureFile(const std::string& signaturesContent)
{
	...
	{
		LOG_ERROR("解析siganature json内容失败. 错误字节 %d", e->byte);
		return;
	}
...
{
	nlohmann::json& jsonContent = *static_cast<nlohmann::json*>(signatureJson);
	for (auto& moduleEntry : jsonContent.items())
	{
		const std::string& moduleName = moduleEntry.key();

		if (!m_ModulePatterns.contains(moduleName))
		...
		{
			const std::string& functionName = functionEntry.key();
			functionPatterns[functionName] = {};

			auto& [signatures, xrefs] = functionPatterns[functionName];
			auto& patternInfo = functionEntry.value();

			for (auto& xref : patternInfo["xref"])
				xrefs.push_back({ xref["sig"], xref["offset"] });
...
void PatternScanner::Save(const std::filesystem::path& filename)
{
	...
	{
		LOG_ERROR("打开文件失败 '%s' 无法保存偏移.", filename.c_str());
		return;
	}
...
bool PatternScanner::Load(const std::filesystem::path& filename)
{
	...
	{
		LOG_ERROR("打开文件失败 '%s' 无法加载偏移.", filename.c_str());
		return false;
	}
...
bool PatternScanner::LoadJson(const nlohmann::json& object)
{
	...
	for (auto& moduleEntry : object.items())
	{
		...
		{
			LOG_WARNING("模块 '%s' 散列值与保存的值不匹配. 似乎模块已更新.", moduleName.c_str());
			result = false;
			continue;
		}

		if (!m_CacheOffsets.contains(moduleName))
...
PatternScanner::ModuleInfo& PatternScanner::GetModuleInfo(HMODULE hModule)
{
	...
	if (s_ModuleInfoCache.contains(hModule))
	...
	if (result == FALSE)
	{
		LOG_LAST_ERROR("获取模块信息失败 0x%p.", hModule);
		std::system("pause");
		exit(0);
	}
	...
	if (result == FALSE)
	{
		LOG_LAST_ERROR("在获取文件名模块失败 0x%p.", hModule);
		std::system("pause");
		exit(0);
	}
	...
	LOG_DEBUG("模块 %s 绑定 0x%p-0x%p.", moduleInfo.filePath.c_str(), 
	...
	while (currentAddress <= endAddress)
	{
		...
		if (byteCount == 0)
		{
			LOG_LAST_ERROR("获取地址内存信息失败 0x%p.", currentAddress);
			break;
		}
...
size_t ComputeChecksum(const std::string& filename)
{
	...
	if (!file.is_open())
	{
		LOG_ERROR("计算文件json失败: %s", filename.c_str());
		return 0;
	}
...
bool PatternScanner::IsValidModuleHash(HMODULE hModule, const nlohmann::json& hashObject)
{
	...
	size_t currChecksum = m_ComputedHashes.contains(hModule) ? m_ComputedHashes[hModule] : ComputeChecksum(GetModuleInfo(hModule).filePath);
...
void PatternScanner::SaveModuleHash(HMODULE hModule, nlohmann::json& outObject)
{
	...
	size_t checksum = m_ComputedHashes.contains(hModule) ? m_ComputedHashes[hModule] : ComputeChecksum(moduleInfo.filePath);
...
void PatternScanner::SearchAll()
{
	for (auto& [moduleName, methodsSignatures] : m_ModulePatterns)
	{
		...
		{
			LOG_DEBUG("搜索 %s::%s", moduleName.c_str(), methodName.c_str());
			auto searchResult = Search(moduleName, methodName);
			if (searchResult == 0)
				LOG_WARNING("没有找到");
			else
				LOG_DEBUG("发现功能 at %s + %p", moduleName.c_str(), searchResult);
		}
	}
}
...
{	
	...
	{
		if (modulePatternsData.contains(name))
...
uintptr_t PatternScanner::Search(const std::string& moduleName, const std::string& name)
{
	...
	if (m_CacheOffsets.contains(moduleName) && m_CacheOffsets[moduleName].contains(name))
	{
		uintptr_t offset = m_CacheOffsets[moduleName][name];
		return offset != 0 ? moduleBase + offset : 0;
	}

	if (m_ModulePatterns.contains(moduleName) && m_ModulePatterns[moduleName].contains(name))
...
uintptr_t PatternScanner::SearchXref(HMODULE hModule, const OffsetSignature& xrefPattern)
{
	...
	case 0x4C:
		...
		break;
	default:
		LOG_WARNING("尝试查找xref到不支持的长调用 (opcode 0x%x)", callOpcode);
		return 0;
	}
...
uintptr_t PatternScanner::SearchInModule(const std::string& moduleName, const std::string& pattern)
{
	...
	if (countFound == 0)
		LOG_WARNING("未找到的模式 '%s'.", pattern.c_str());
...
uintptr_t PatternScanner::SearchInModule(HMODULE hModule, const std::string& pattern)
{
	...
	if (count == 0)
	{
		LOG_ERROR("获取模块名称失败 0x%p.", hModule);
		return {};
	}
