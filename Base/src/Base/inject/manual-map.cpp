...
bool ManualMapDLL(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason) {
	...
	if (!pShellcode) {
		ILogError("[DLL注入]内存shellcode分配失败 (ex).\n");
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		return false;
	}
	...
	ILog("[DLL注入]映射的DLL在 %p\n", pTargetBase);
	ILog("[DLL注入]映射信息在 %p\n", MappingDataAlloc);
	ILog("[DLL注入]ShellCode在 %p\n", pShellcode);

	ILog("[DLL注入]数据分配\n");

#ifdef _DEBUG
	ILog("[DLL注入]我的shellcode指针 %p\n", Shellcode);
	ILog("[DLL注入]目标点 %p\n", pShellcode);
#endif
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				...
				if (VirtualProtectEx(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSectionHeader->Misc.VirtualSize, newP, &old)) {
					ILog("[DLL注入]节 %s 设置为 %lX\n", (char*)pSectionHeader->Name, newP);
				}
				else {
					ILog("[DLL注入]失败:节 %s 未设置为 %lX\n", (char*)pSectionHeader->Name, newP);
				}
			}
		}
		DWORD old = 0;
