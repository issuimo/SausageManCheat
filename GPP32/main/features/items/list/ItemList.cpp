...
auto ItemList::GetInfo() const -> const GuiInfo& { return *new GuiInfo{ reinterpret_cast<const char*>(u8"物品"), reinterpret_cast<const char*>(u8"列表"), true, false, false }; }
...
void ItemList::Render() {
	...
	if (ImGui::BeginTable("List", 2,
		...
		ImGui::TableSetupColumn(reinterpret_cast<const char*>(u8"数量"), ImGuiTableColumnFlags_None);
		...
		for (const auto& obj : PickItem::vector) {
			...
			try {
				if (!IsBadReadPtr(obj, sizeof(PickItem)) && !IsBadReadPtr(obj->pickItemData, sizeof(PickItemDataConfig))) {
					if (ImGui::TableSetColumnIndex(0)) ImGui::Text(std::format("{}", obj->checkNum).c_str());
					if (ImGui::TableSetColumnIndex(1)) ImGui::Text(std::format("{}", obj->pickItemData->itemName->ToString()).c_str());
				}
			} catch (...) {
