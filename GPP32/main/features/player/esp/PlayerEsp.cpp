			...
			[&] {
				__try {
					[&] {
						if (Role::localRole) {
							II::Vector3 mPoint;
							if (Role::localRole && !IsBadReadPtr(Role::localRole, sizeof(RoleLogic))) mPoint = Role::localRole->roleLogic->NowPoint;
							...
							for (const auto& player : Role::vector) {
								...
								[&] {
									__try {
										[&] {
											if (player == Role::localRole || IsBadReadPtr(Role::localRole, sizeof(RoleLogic)) || IsBadReadPtr(player, sizeof(Role)) || IsBadReadPtr(player->MyRoleControl, sizeof(RoleControl)) || IsBadReadPtr(player->roleLogic, sizeof(RoleLogic)) || Role::localRole->roleLogic->TeamNum == player->roleLogic->TeamNum) return;
											...
											[&] {
												...
												} __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("RoleEsp-GetData Error (except)"); }(); }
											}();
											...
											TIME_END(1);
											TIME_UPDATA("PlayerEsp::Role::GetData", 1);
											if (auto sPoint = WorldToScreenPoint(point); sPoint.z > 0) {
												...
												TIME_END(2);
												TIME_UPDATA("PlayerEsp::Role::Box", 2);
												...
												TIME_END(3);
												TIME_UPDATA("PlayerEsp::Role::Line", 3);
												...
												if (setting.bone && !weak) {
													if (auto animator = player->MyRoleControl->animatorControl->animator) {
														...
														[&] {
															...
															} __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("RoleEsp-AnimatorBone Error (except)"); }(); }
														}();
...
#define B(x) x.z > -1
												...
												TIME_END(4);
												TIME_UPDATA("PlayerEsp::Role::Bone", 4);
													...
													[&] {
														__try {
															[&] {
																...
																if (setting.part) {
																	const std::string headPart = player->roleLogic->HeadEquipPart ? player->roleLogic->HeadEquipPart->MyPickItemData->itemName->ToString() : GbkToUtf8("无");
																	const std::string bodyPart = player->roleLogic->BodyEquipPart ? player->roleLogic->BodyEquipPart->MyPickItemData->itemName->ToString() : GbkToUtf8("无");
																	const std::string packPart = player->roleLogic->PackEquip ? player->roleLogic->PackEquip->MyPickItemData->itemName->ToString() : GbkToUtf8("无");
														...
														} __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("PlayerEsp Error (for) (info) (except)"); }(); }
													}();
												}
												TIME_END(5);
												TIME_UPDATA("PlayerEsp::Role::Info", 5);
											}
										}();
									} __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("PlayerEsp Error (for) (except)"); }(); }
								}();
							}
						}
					}();
				} __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("PlayerEsp Error (except)"); }(); }
			}();
		} catch (...) { ERROR("PlayerEsp->top->nullptr") }
	}
}
...
auto PlayerEsp::Render() -> void {
	...
	if (enable && ImGui::BeginTable("PlayerEsp", 2, ImGuiTableFlags_ScrollX | ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV | ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable, ImVec2(0.0F, ImGui::GetTextLineHeightWithSpacing() * 8))) {
		...
		if (ImGui::TableSetColumnIndex(1)) ImGui::Text((const char*)u8"倒地");
		ImGui::PopID();

		ImGui::PushID("朝向ch");
		ImGui::TableNextRow();
		if (ImGui::TableSetColumnIndex(0)) ImGui::Checkbox("", &setting.viewLook);
		if (ImGui::TableSetColumnIndex(1)) ImGui::Text((const char*)u8"朝向");
		ImGui::PopID();
