	...
	if (enable) {
		...
		try {
			...
			for (const auto& player : Role::vector) {
				if (player == Role::localRole || IsBadReadPtr(player, sizeof(Role)) || IsBadReadPtr(player->MyRoleControl, sizeof(RoleControl)) || IsBadReadPtr(player->roleLogic, sizeof(RoleLogic))) continue;

				auto       point = player->roleLogic->NowPoint;
				const int  team = player->roleLogic->TeamNum;
				auto scale = player->GetTransform()->GetLocalScale();
				const float height = 1.6 * scale.y;
				const float radius = 0.4 * scale.x;
...
#define S(x) x.y = static_cast<float>(windowHeight) - x.y
					...
					if (setting.info) {
						...
						DrawTextWithOutline(bg, { xy.x - 95.0f, xy.y - 25.0f }, std::format("{:02} | {}", team, team ? player->roleLogic->NickName->ToString() : GbkToUtf8("[人机] ") + player->roleLogic->NickName->ToString()).c_str(), ImColor{255,255,255}, 1, DrawHelp::OutlineSide::All, ImColor{0,0,0});
						DrawTextWithOutline(bg, { xy.x - 100.0f, xy.y - 45.0f }, (std::format("{}m | {}", (int)sqrt(pow(point.x - mPoint.x, 2) + pow(point.y - mPoint.y, 2) + pow(point.z - mPoint.z, 2)), player->roleLogic->UserWeapon ? player->roleLogic->UserWeapon->MyPickItemData->itemName->ToString() : GbkToUtf8("空手"))).c_str(), ImColor{255,255,255}, 1, DrawHelp::OutlineSide::All, ImColor{0,0,0});
					}
				}
			}
