...
auto Main::Init(const HMODULE hModule) -> void {
	...
	dx_hook::Hk11::Build([&] {
		...
		if (show) {
			...
			if (ImGui::Begin(("Atom by 遂沫(github@issuimo)"), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove)) {
				if (ImGui::Button("保存")) {
				...
				ImGui::SameLine();
				if (ImGui::Button("转储存")) I::DumpToFile("./");
		...
		if (ImGui::Begin("Draw (don`t selected)", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav)) {
			...
			DrawTextWithOutline(bg, {5, static_cast<float>(windowHeight - 20)}, "作者：遂沫 | github：issuimoo | mail: 1992724048@qq.com | QQ: 1992724048 | Telegram: ISSUIMO | QQ群聊: 472659840 | 版本: 243212", ImColor{255, 255, 255}, 1, DrawHelp::OutlineSide::All, ImColor{0, 0, 0});
			for (const auto& feature : Feature::features | std::views::values) {
				for (const auto func : feature) {
					if (func->GetInfo().needDraw) {
						...
						func->Draw();
						TIME_END(2)
						TIME_UPDATA(func->GetInfo().tableName + ":" + func->GetInfo().groupName, 2)
					}
				}
			}
	...
	std::thread([&] {
		...
		while (true) {
			...
			if (hmodule) {
				try {
					const std::uint32_t base = *reinterpret_cast<std::uint32_t*>(reinterpret_cast<std::uint32_t>(hmodule) + 0x12C6558);
					if (!base) continue;
					const std::uint32_t p1 = *reinterpret_cast<std::uint32_t*>(base + 0x5C);
					if (!p1) continue;
					const std::uint32_t p2 = *reinterpret_cast<std::uint32_t*>(p1 + 0x10);
					if (!p2) continue;
					const std::uint32_t p3 = *reinterpret_cast<std::uint32_t*>(p2 + 0x10);
					if (!p3) continue;
					const std::uint32_t p4 = *reinterpret_cast<std::uint32_t*>(p3 + 0x8);
					if (!p4) continue;
					matrix = *reinterpret_cast<II::Matrix4x4*>(p4 + 0x2CC);
				} catch (...) { ERROR("GetMatrix4x4 (catch)") }
			} else hmodule = GetModuleHandle(L"UnityPlayer.dll");
		}
	}).detach();
	LOG_INFO("启动数据更新线程完成!\n");
