...
public:
	...
	static auto Update() -> void {
		...
		if (pClass) {
			...
			[&] { __try { [&]() { for (const auto item : pClass->FindObjectsByType<PickItem*>()) { if (!IsBadReadPtr(item, sizeof(PickItem)) && !IsBadReadPtr(item->MyPickItemNet, sizeof(PickItemNet)) && !IsBadReadPtr(item->pickItemData, sizeof(PickItemDataConfig))) temp.push_back(item); } }(); } __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("PickItem->FindObjectsByType (except)"); }(); } }();
