					...
					[&] {
						for (const auto pRole : pClass->FindObjectsByType<Role*>()) {
							if (!IsBadReadPtr(pRole, sizeof(Role))) {
								if (!IsBadReadPtr(pRole->roleLogic, sizeof(RoleLogic)) && pRole->roleLogic->isLocalRole) local = pRole;
								temp.push_back(pRole);
							}
						}
					}();
				} __except (EXCEPTION_EXECUTE_HANDLER) { [] { ERROR("Role-FindObjectsByType (except)"); }(); }
			}();
