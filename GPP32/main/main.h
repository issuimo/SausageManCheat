...
public:
	...
	[[nodiscard]] virtual auto GetInfo() const -> const GuiInfo& { return *new GuiInfo{reinterpret_cast<const char*>("未定义"), reinterpret_cast<const char*>(""), false, false, false}; } ;
...
inline static auto GbkToUtf8(const std::string& str) -> std::string {
...
inline static auto Utf8ToGbk(const std::string& str) -> std::string {
...
inline static auto RotatePoint(const float x, const float y, const float xb, const float yb, const float theta, float& x1, float& y1) -> void {
	// 将角度转换为弧度
	...
	// 计算坐标差值
	const float deltaX = x - xb;
	const float deltaY = y - yb;

	// 计算旋转后的坐标
	x1 = deltaX * cosValue - deltaY * sinValue + xb;
	y1 = deltaY * cosValue + deltaX * sinValue + yb;
}

inline static II::Vector3 CalcAngles(const II::Vector3& us, II::Vector3 them) {
	const II::Vector3 dists = them - us;
	const float hyp   = sqrt((dists.x * dists.x) + (dists.y * dists.y) + (dists.z * dists.z));
	II::Vector3 result;
	result.x = (asinf(dists.z / hyp) * 57.295779513082f);
	result.y = (atanf(dists.y / dists.x) * 57.295779513082f);
	result.z = 0;
	if (dists.x <= 0) result.y += 180;
	result.x = -result.x;
	return result;
}

inline static auto WorldToScreenPoint(const II::Vector3& WorldLocation) -> II::Vector3 {
	...
	float y     = Main::windowHeight / 2 - (Main::matrix[0][1] * WorldLocation.x + Main::matrix[1][1] * WorldLocation.y + Main::matrix[2][1] * WorldLocation.z + Main::matrix[3][1]) * ViewW * Main::windowHeight / 2;
	return {x, y, ViewW};
}
...
namespace DrawHelp {
	...
	static auto operator&(OutlineSide lhs, OutlineSide rhs) -> bool { return static_cast<std::underlying_type_t<OutlineSide>>(lhs) & static_cast<std::underlying_type_t<OutlineSide>>(rhs); }
